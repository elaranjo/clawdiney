...
3. **MCP Server** - Provides integration with Codex via Model Context Protocol

Data flow:
Obsidian Vault → brain_indexer.py → ChromaDB + Neo4j → brain_mcp_server.py → Codex
...
- `brain_mcp_server.py` - Main MCP server that integrates with Codex (with context manager support)
